...
def create_retriever():
    ...
    DB_PATH = "C:\\RAG\\langchain-kr\\00-Mine\\streamlit_myProject\\MultiVector_plain_final_db"
    STORE_PATH = "C:\\RAG\\langchain-kr\\00-Mine\\streamlit_myProject\\local_docstore"
    id_key = "doc_id"
    ...
    def load_split_docs(
        file_path="C:\\RAG\langchain-kr\\00-Mine\\streamlit_myProject\\split_documents.pkl",
    ):
        """저장된 분할 문서들을 로드"""
    ...
    with open(
        "C:\\RAG\\langchain-kr\\00-Mine\\streamlit_myProject\\doc_ids.pkl", "rb"
    ) as f:
    ...
    def kiwi_tokenize(text):
        # Kiwi tokenizer returns a list of Token objects
        # We need to extract just the morphemes
        tokens = kiwi.tokenize(text)
        # Extract just the morphemes from each token
        morphemes = []
        for token in tokens:
            if isinstance(token, list):  # If token is already a list
                morphemes.extend([t for t in token])
            else:  # If token is a Token object
                morphemes.append(token.form)
        return morphemes

    # bm25 retriever
    bm25_kiwi_retriever = BM25Retriever.from_documents(
        docs, preprocess_func=kiwi.tokenize
    )
    ...
    rerank_compressor = CrossEncoderReranker(
        model=HuggingFaceCrossEncoder(model_name="BAAI/bge-reranker-v2-m3"), top_n=2
    )
